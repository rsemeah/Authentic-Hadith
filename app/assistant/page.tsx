...
function AssistantPageContent() {
  ...
  const sendMessage = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setMessages((prev: ChatMessage[]) => [...prev, { role: 'assistant', content: data.reply, aiMessageId: data.aiMessageId }]);
    } catch {
      setError('Something went wrong. Please try again in a moment.');
    } finally {
      setSending(false);
      ...
      <section className="rounded-2xl bg-white p-4 text-xs text-neutral-600 shadow-sm ring-1 ring-neutral-100">
        ...
        {sessionId && (
          <div className="mt-2 flex items-center gap-3 text-[11px] text-neutral-500">
            ...
            <span>Conversation saved (session {sessionId.slice(0, 6)}…)</span>
            <button
              type="button"
              onClick={() => {
                setMessages([]);
              ...
              className="rounded-full border border-neutral-200 px-2 py-1 text-[11px] font-medium text-neutral-700 hover:border-neutral-900"
            >
              Start fresh
            </button>
          </div>
